...
uses DMTD, Menu, QkRelPro, QkProDef, FuncGeral, ProcGeral;
...
Procedure TF_ProDef.GerarArquivoTxt(Sender : TOBject);
var
...
begin
 ...
 // arqu := Trim(Copy(ComboBox1.Text, 7, 3)) + TrimChar(Copy(maskedit1.text,1,5),'/') + Copy(combobox2.text,1,2) + '.TXT';
  arqu := cam + '\' + arqu;
  { Edson inicio }
  F_ProDef.SaveDialog1.FileName := arqu;
  F_ProDef.SaveDialog1.Execute;
  arqu := F_ProDef.SaveDialog1.FileName;
  { Edson fim }
  AssignFile(F1, arqu);
  ReWrite(F1);
  linha := '';
...
procedure TF_ProDef.BitBtn3Click(Sender: TObject);
begin
  Close;
end;

procedure TF_ProDef.BitBtn2Click(Sender: TObject);
begin
  MaskEdit1.Clear;
...
procedure TF_ProDef.BitBtn1Click(Sender: TObject);
var
  arq : String;
  contrato : String;
begin
 ...
 // DM.Join.Sql.Add('AND (a.sit_prov3   = ' + Chr(39) + tpbx + Chr(39) + '))');}
  ...
  begin
    GerarArquivoTxt(F_ProDef);
  end
  else
   begin
    Application.CreateForm(TQk_ProDef, Qk_ProDef);
    Qk_ProDef.QrLabel1.Caption := F_Menu.Label29.Caption;
    Qk_ProDef.QrLabel5.Caption := 'RELATÓRIO GERAL DAS NOTIFICAÇÕES NO PERÍODO  DE  ' + MaskEdit1.Text+ ' ATÉ ' +MaskEdit2.Text ;
    Qk_ProDef.QuickRep1.Preview;
    Qk_ProDef.Destroy;
    Qk_ProDef := NIL;
   end;
  Dm.ArqAux.Close;
...
procedure TF_ProDef.MaskEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then Maskedit2.SetFocus;
end;

procedure TF_ProDef.MaskEdit2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
    if key = 13 then
  begin
...
procedure TF_ProDef.ComboBox1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TF_ProDef.ComboBox2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then BitBtn1.SetFocus;
end;

procedure TF_ProDef.ComboBox2Exit(Sender: TObject);
begin
//  TpBx := 'P';
//  TpBx := Copy(ComboBox2.Text, 1,1);
end;

procedure TF_ProDef.ComboBox1Enter(Sender: TObject);
begin
  SendMessage(ComboBox1.handle, CB_SHOWDROPDOWN, Integer(True), 0);
end;

procedure TF_ProDef.ComboBox2Enter(Sender: TObject);
begin
//  SendMessage(ComboBox2.handle, CB_SHOWDROPDOWN, Integer(True), 0);
end;

procedure TF_ProDef.MaskEdit3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
    if key = 13 then MaskEdit4.SetFocus;
end;

procedure TF_ProDef.MaskEdit4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
    if key = 13 then Maskedit1.setfocus;
end;

procedure TF_ProDef.FormActivate(Sender: TObject);
begin
    F_ProDef.MaskEdit3.SetFocus;
end;
