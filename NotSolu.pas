...
var
  F_NotSolu: TF_NotSolu;
  tpbx    : String;
  F1      : TextFile;

implementation

uses DMTD, Menu, QkRetPad, QkRetI, RRet, QkRetPad1, QNotSolu, FuncGeral, ProcGeral;
...
Procedure TF_NotSolu.GerarArquivoTxt(Sender : TOBject);
var
...
begin
  ...
  arqu := cam + '\' + arqu;
  { Edson inicio }
  F_NotSolu.SaveDialog1.FileName := arqu;
  F_NotSolu.SaveDialog1.Execute;
  arqu := F_NotSolu.SaveDialog1.FileName;
  { Edson fim }
  AssignFile(F1, arqu);
  ReWrite(F1);
  linha := '';
...
procedure TF_NotSolu.BitBtn3Click(Sender: TObject);
begin
  Close;
end;

procedure TF_NotSolu.BitBtn2Click(Sender: TObject);
begin
  MaskEdit1.Clear;
...
procedure TF_NotSolu.BitBtn1Click(Sender: TObject);
var
  arq : String;
  contrato : String;
begin
  ...
  DM.Join.Sql.Add('a.sit_not1, a.sit_not2, a.sit_not3, a.cod_ir1, a.cod_ir2, a.cod_ir3,a.rl,');
 ...
 // DM.Join.Sql.Add('AND a.sit_not3   = ' + Chr(39) + tpbx + Chr(39) + ')');
  ...
  begin
    GerarArquivoTxt(F_NotSolu);
  end
  else
   begin
    Application.CreateForm(TQk_NotSolu, Qk_NotSolu);
    Qk_NotSolu.QrLabel1.Caption := F_Menu.Label29.Caption;
    Qk_NotSolu.QrLabel5.Caption := 'RELAÇÃO DAS NOTIFICAÇÕES SOLUCIONADAS NO PERÍODO DE  ' + MaskEdit1.Text+ ' ATÉ ' +MaskEdit2.Text ;
    Qk_NotSolu.QuickRep1.Preview;
    Qk_NotSolu.Destroy;
    Qk_NotSolu := NIL;
   end;
  Dm.ArqAux.Close;
...
procedure TF_NotSolu.MaskEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then Maskedit2.SetFocus;
end;

procedure TF_NotSolu.MaskEdit2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
    if key = 13 then
  begin
...
procedure TF_NotSolu.ComboBox1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then BitBtn1.SetFocus;
end;

procedure TF_NotSolu.ComboBox2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then BitBtn1.SetFocus;
end;

procedure TF_NotSolu.ComboBox1Enter(Sender: TObject);
begin
  SendMessage(ComboBox1.handle, CB_SHOWDROPDOWN, Integer(True), 0);
end;

procedure TF_NotSolu.ComboBox2Enter(Sender: TObject);
begin
