...
uses DMTD, Menu, QkRelPr1, RelPro, FuncGeral, ProcGeral;
...
Procedure TF_RelPro1.GerarArquivoTxt(Sender : TOBject);
var
...
begin
 // if (StrToInt(Copy(ComboBox1.Text, 1,3)) = 6) then cam := F_Menu.Label27.Caption + 'Intercap\' else cam := F_Menu.Label27.Caption;
  ...
  arqu := cam + '\' + arqu;
  { Edson inicio }
  F_RelPro1.SaveDialog1.FileName := arqu;
  F_RelPro1.SaveDialog1.Execute;
  arqu := F_RelPro.SaveDialog1.FileName;
  { Edson fim }
  AssignFile(F1, arqu);
  ReWrite(F1);
  linha := '';
...
//  linha := '| REGISTRO | DT. REGISTRO | PROTOCOLO | DESTINATÁRIO                             |  ENDERECO                                |  RL       |  OCORRENCIA    |  CONTRATO |  PARCELA  |';
  ...
  begin
   ...
   // linha := linha + '| ' + StrBranco(Dm.ArqAux.FieldByName('rl').asString, 10, 'D');
    ...
    linha := linha + '|';
    Grava_Linha(linha);
    if (Trim(Dm.ArqAux.FieldByName('destinatario').asString) <> '') then
       qde := qde + 1;
    Dm.ArqAux.Next;
  end;
  linha := StringOfChar('-', 200);
...
procedure TF_RelPro1.BitBtn3Click(Sender: TObject);
begin
  Close;
end;

procedure TF_RelPro1.BitBtn2Click(Sender: TObject);
begin
  MaskEdit1.Clear;
...
procedure TF_RelPro1.BitBtn1Click(Sender: TObject);
var
  arq : String;
  contrato : String;
begin
 ...
 // DM.Join.Sql.Add('and (a.sit_prov1 = "N")');
   ...
   begin
      ...
      DM.Join.Sql.Add('AND ((a.data_prov1 >= ' + Chr(39) + ConvData(MaskEdit3.Text) + Chr(39)  +'AND a.data_prov1 <= ' + Chr(39) + ConvData(MaskEdit4.Text) + Chr(39)+'and (a.sit_prov1 = "N"))');
      DM.Join.Sql.Add('or (a.data_prov2 >= ' + Chr(39) + ConvData(MaskEdit3.Text) + Chr(39)  +'AND a.data_prov2 <= ' + Chr(39) + ConvData(MaskEdit4.Text) + Chr(39)+'and (a.sit_prov2 = "N"))');
      DM.Join.Sql.Add('or (a.data_prov3 >= ' + Chr(39) + ConvData(MaskEdit3.Text) + Chr(39)  +'AND a.data_prov3 <= ' + Chr(39) + ConvData(MaskEdit4.Text) + Chr(39)+'and (a.sit_prov3 = "N")))');
 ...
 //Verificando a primeria baixa, se é ou não negativa
 // if (DM.Join.FieldByName('sit_prov1').asString = 'N') then
  // begin
    ...
    if (DM.Join.FieldByName('data_prov3').asString <> '') and (DM.Join.FieldByName('sit_prov3').asString = 'N') then
    begin
    ...
    else if (DM.Join.FieldByName('data_prov2').asString <> '') and (DM.Join.FieldByName('sit_prov2').asString = 'N') then
    begin
       Dm.ArqAux.FieldByName('ocorrencia').asString := 'NEGATIVA';
    ...
    else if (DM.Join.FieldByName('data_prov1').asString <> '') and (DM.Join.FieldByName('sit_prov1').asString = 'N') then
    begin
       Dm.ArqAux.FieldByName('ocorrencia').asString := 'NEGATIVA';
   ...
   // dias_uteis := currtostr((Dm.ArqAux.FieldByName('data_prov1').asdatetime) - (Dm.ArqAux.FieldByName('data_p').asdatetime));
    if dias_uteis < 0 then
       dias_uteis := 0;
    Dm.ArqAux.FieldByName('dias_uteis').asstring := inttostr(dias_uteis);
    Dm.ArqAux.Post;
  //  end;

 { //Verificando a segunda baixa, se  é ou não negativa
  if (DM.Join.FieldByName('sit_prov2').asString = 'N') then
   begin
    ...
    if (DM.Join.FieldByName('data_prov2').asString <> '') and (DM.Join.FieldByName('sit_prov2').asString = 'N') then
     begin
      Dm.ArqAux.FieldByName('ocorrencia').asString := 'NEGATIVA';
      Dm.ArqAux.FieldbyName('motivo').asString     := Ver_Ocorrencia(DM.Join.FieldByName('cod_prov2').asString);
      Dm.ArqAux.FieldByName('data_prov1').asString    := DM.Join.FieldByName('data_prov2').asString;
    ...
    if (DM.Join.FieldByName('data_prov1').asString <> '') and (DM.Join.FieldByName('sit_prov1').asString = 'N') then
     begin
      Dm.ArqAux.FieldByName('ocorrencia').asString := 'NEGATIVA';
      Dm.ArqAux.FieldbyName('motivo').asString     := Ver_Ocorrencia(DM.Join.FieldByName('cod_prov1').asString);
      Dm.ArqAux.FieldByName('data_prov1').asString    := DM.Join.FieldByName('data_prov1').asString;
   ...
   // dias_uteis := currtostr((Dm.ArqAux.FieldByName('data_prov1').asdatetime) - (Dm.ArqAux.FieldByName('data_p').asdatetime));
    ...
    //Verificando se a terceira baixa, é ou não negativa
   if (DM.Join.FieldByName('sit_prov1').asString = 'N') then
   begin
    Dm.ArqAux.Append;
    Dm.ArqAux.FieldByName('indexador').asInteger    := DM.Join.FieldByName('registro').asInteger;
  ...
  if (CheckBox1.Checked) then
    GerarArquivoTxt(F_RelPro1)
  else
  begin
     Application.CreateForm(TQk_RelPro1, Qk_RelPro1);
     Qk_RelPro1.QrLabel1.Caption := F_Menu.Label29.Caption;
     Qk_RelPro1.QrLabel5.Caption := 'RELATÓRIO GERAL DAS NOTIFICAÇÕES NO PERÍODO  DE  ' + MaskEdit1.Text+ ' ATÉ ' +MaskEdit2.Text ;
     Qk_RelPro1.QuickRep1.Preview;
     Qk_RelPro1.Destroy;
     Qk_RelPro1 := NIL;
  end;
  Dm.ArqAux.Close;
...
procedure TF_RelPro1.MaskEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then Maskedit2.SetFocus;
end;

procedure TF_RelPro1.MaskEdit2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
    if key = 13 then
  begin
...
procedure TF_RelPro1.ComboBox1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TF_RelPro1.ComboBox2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then BitBtn1.SetFocus;
end;

procedure TF_RelPro1.ComboBox2Exit(Sender: TObject);
begin
//  TpBx := 'P';
  TpBx := Copy(ComboBox2.Text, 1,1);
end;

procedure TF_RelPro1.ComboBox1Enter(Sender: TObject);
begin
  SendMessage(ComboBox1.handle, CB_SHOWDROPDOWN, Integer(True), 0);
end;

procedure TF_RelPro1.ComboBox2Enter(Sender: TObject);
begin
  SendMessage(ComboBox2.handle, CB_SHOWDROPDOWN, Integer(True), 0);
end;

procedure TF_RelPro1.MaskEdit3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then MaskEdit4.SetFocus;
end;

procedure TF_RelPro1.MaskEdit4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key = 13 then MaskEdit1.SetFocus;
end;

procedure TF_RelPro1.FormActivate(Sender: TObject);
begin
   F_RelPro1.MaskEdit3.SetFocus;
end;
