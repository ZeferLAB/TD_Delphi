...
uses DMTD, Menu, QkRelPr7, FuncGeral, ProcGeral;
...
Procedure TF_RelPro7.GerarArquivoTxt(Sender : TOBject);
var
...
begin
 // if (StrToInt(Copy(ComboBox1.Text, 1,3)) = 6) then cam := F_Menu.Label27.Caption + 'Intercap\' else cam := F_Menu.Label27.Caption;
  ...
  arqu := cam + '\' + arqu;
  { Edson inicio }
  F_RelPro7.SaveDialog1.FileName := arqu;
  F_RelPro7.SaveDialog1.Execute;
  arqu := F_RelPro7.SaveDialog1.FileName;
  { Edson fim }
  AssignFile(F1, arqu);
  ReWrite(F1);
  linha := '';
...
procedure TF_RelPro7.BitBtn3Click(Sender: TObject);
begin
  Close;
end;

procedure TF_RelPro7.BitBtn2Click(Sender: TObject);
begin
  MaskEdit1.Clear;
...
procedure TF_RelPro7.BitBtn1Click(Sender: TObject);
var
  arq : String;
  contrato : String;
begin
 ...
 // DM.Join.Sql.Add('AND (a.sit_prov3   = ' + Chr(39) + tpbx + Chr(39) + '))');}
  ...
  begin
    GerarArquivoTxt(F_RelPro7);
  end
  else
   begin
    Application.CreateForm(TQk_RelPro7, Qk_RelPro7);
    Qk_RelPro7.QrLabel1.Caption := F_Menu.Label29.Caption;
    Qk_RelPro7.QrLabel5.Caption := 'RELATÓRIO GERAL DAS NOTIFICAÇÕES NO PERÍODO  DE  ' + MaskEdit1.Text+ ' ATÉ ' +MaskEdit2.Text ;
    Qk_RelPro7.QuickRep1.Preview;
    Qk_RelPro7.Destroy;
    Qk_RelPro7 := NIL;
   end;
  Dm.ArqAux.Close;
...
procedure TF_RelPro7.MaskEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then Maskedit2.SetFocus;
end;

procedure TF_RelPro7.MaskEdit2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
    if key = 13 then
  begin
...
procedure TF_RelPro7.ComboBox1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TF_RelPro7.ComboBox2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then BitBtn1.SetFocus;
end;

procedure TF_RelPro7.ComboBox2Exit(Sender: TObject);
begin
//  TpBx := 'P';
  TpBx := Copy(ComboBox2.Text, 1,1);
end;

procedure TF_RelPro7.ComboBox1Enter(Sender: TObject);
begin
  SendMessage(ComboBox1.handle, CB_SHOWDROPDOWN, Integer(True), 0);
end;

procedure TF_RelPro7.ComboBox2Enter(Sender: TObject);
begin
  SendMessage(ComboBox2.handle, CB_SHOWDROPDOWN, Integer(True), 0);
end;

procedure TF_RelPro7.MaskEdit3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key = 13 then MaskEdit4.SetFocus;
end;

procedure TF_RelPro7.MaskEdit4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
    if key = 13 then MaskEdit1.SetFocus;
end;

procedure TF_RelPro7.FormActivate(Sender: TObject);
begin
  F_RelPro7.MaskEdit3.SetFocus;
end;
