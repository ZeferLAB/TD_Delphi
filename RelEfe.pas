...
uses DMTD, Menu, QkRelEfe, FuncGeral, ProcGeral;
...
Procedure TF_RelEfe.GerarArquivoTxt(Sender : TOBject);
var
...
begin
 ...
 // arqu := Trim(Copy(ComboBox1.Text, 7, 3)) + TrimChar(Copy(maskedit1.text,1,5),'/') + Copy(combobox2.text,1,2) + '.TXT';
  arqu := cam + '\' + arqu;
  { Edson inicio }
  F_RelEfe.SaveDialog1.FileName := arqu;
  F_RelEfe.SaveDialog1.Execute;
  arqu := F_RelEfe.SaveDialog1.FileName;
  { Edson fim }
  AssignFile(F1, arqu);
  ReWrite(F1);
  linha := '';
...
procedure TF_RelEfe.BitBtn3Click(Sender: TObject);
begin
  Close;
end;

procedure TF_RelEfe.BitBtn2Click(Sender: TObject);
begin
  MaskEdit1.Clear;
...
procedure TF_RelEfe.BitBtn1Click(Sender: TObject);
var
  arq : String;
  contrato : String;
begin
  ...
  DM.Join.Sql.Add('a.grupo, a.cota, a.data_impr, a.data_reti, a.protocolo,');
 ...
 // DM.Join.Sql.Add('FROM td');
  ...
  if radiobutton1.checked = true then
   begin
    DM.Join.Sql.Add('AND ((a.data_impr >= ' + Chr(39) + ConvData(MaskEdit1.Text) + Chr(39)  +'AND a.data_impr <= ' + Chr(39) + ConvData(MaskEdit1.Text) + Chr(39)+')');
 ...
 // DM.Join.Sql.Add('AND (a.sit_prov3   = ' + Chr(39) + tpbx + Chr(39) + '))');}
  ...
  begin
    GerarArquivoTxt(F_RelEfe);
  end
  else
   begin
    Application.CreateForm(TQk_RelEfe, Qk_RelEfe);
    Qk_RelEfe.QrLabel1.Caption := F_Menu.Label29.Caption;
    Qk_RelEfe.QrLabel5.Caption := 'RELATÓRIO GERAL DAS NOTIFICAÇÕES NO PERÍODO  DE  ' + MaskEdit1.Text+ ' ATÉ ' +MaskEdit2.Text ;
    Qk_RelEfe.QuickRep1.Preview;
    Qk_RelEfe.Destroy;
    Qk_RelEfe := NIL;
   end;
  Dm.ArqAux.Close;
...
procedure TF_RelEfe.MaskEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then Maskedit2.SetFocus;
end;

procedure TF_RelEfe.MaskEdit2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if key = 13 then
  begin
...
procedure TF_RelEfe.ComboBox1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TF_RelEfe.ComboBox2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then BitBtn1.SetFocus;
end;

procedure TF_RelEfe.ComboBox2Exit(Sender: TObject);
begin
//  TpBx := 'P';
//  TpBx := Copy(ComboBox2.Text, 1,1);
end;

procedure TF_RelEfe.ComboBox1Enter(Sender: TObject);
begin
  SendMessage(ComboBox1.handle, CB_SHOWDROPDOWN, Integer(True), 0);
end;

procedure TF_RelEfe.ComboBox2Enter(Sender: TObject);
begin
//  SendMessage(ComboBox2.handle, CB_SHOWDROPDOWN, Integer(True), 0);
end;

procedure TF_RelEfe.MaskEdit3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
    if key = 13 then MaskEdit4.SetFocus;
end;

procedure TF_RelEfe.MaskEdit4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
