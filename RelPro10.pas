...
uses DMTD, Menu, QkRelPr2, QkRelP10, FuncGeral, ProcGeral;
...
Procedure TF_RelPro10.GerarArquivoTxt(Sender : TOBject);
var
...
begin
 // if (StrToInt(Copy(ComboBox1.Text, 1,3)) = 6) then cam := F_Menu.Label27.Caption + 'Intercap\' else cam := F_Menu.Label27.Caption;
  ...
  arqu := cam + '\' + arqu;
  { Edson inicio }
  F_RelPro10.SaveDialog1.FileName := arqu;
  F_RelPro10.SaveDialog1.Execute;
  arqu := F_RelPro10.SaveDialog1.FileName;
  { Edson fim }
  AssignFile(F1, arqu);
  ReWrite(F1);
  linha := '';
...
procedure TF_RelPro10.BitBtn3Click(Sender: TObject);
begin
  Close;
end;

procedure TF_RelPro10.BitBtn2Click(Sender: TObject);
begin
  MaskEdit1.Clear;
...
procedure TF_RelPro10.BitBtn1Click(Sender: TObject);
var
  arq : String;
  contrato : String;
begin
 ...
 // DM.Join.Sql.Add('AND (a.sit_prov3   = ' + Chr(39) + tpbx + Chr(39) + '))');}
  ...
  begin
   ...
   if (DM.Join.FieldByName('data_prov3').asString <> '') then
    ...
    // Cálculo de dias Úteis entre a data de baixa provisória e a data de registro
    dias_uteis    := 0;
    fim_de_semana := 0;
   ...
   // Cálculo de dias Úteis entre a data de baixa definitiva e a data de registro
    dias_uteis2   := 0;
    fim_de_semana := 0;
    dias_uteis2 := strtoint(currtostr((Dm.ArqAux.FieldByName('data_not1').asdatetime) - (Dm.ArqAux.FieldByName('data_p').asdatetime)));
    dt := Dm.ArqAux.FieldByName('data_p').asdatetime;
    for x := 1 to dias_uteis2 do
     begin
      dt     := dt + 1;
      if ((DayOfWeek(dt) = 1) or (DayOfWeek(dt) = 7)) then
       fim_de_semana := fim_de_semana + 1;
     end;
    dias_uteis2:= dias_uteis2 - fim_de_semana;
   // dias_uteis := currtostr((Dm.ArqAux.FieldByName('data_prov1').asdatetime) - (Dm.ArqAux.FieldByName('data_p').asdatetime));
    if dias_uteis2 < 0 then dias_uteis2 := 0;
    Dm.ArqAux.FieldByName('dias_uteis2').asstring := inttostr(dias_uteis2);
  ...
  begin
    GerarArquivoTxt(F_RelPro10);
  end
  else
   begin
    Application.CreateForm(TQk_RelPro10, Qk_RelPro10);
    Qk_RelPro10.QrLabel1.Caption := F_Menu.Label29.Caption;
    Qk_RelPro10.QrLabel5.Caption := 'RELATÓRIO GERAL DAS NOTIFICAÇÕES NO PERÍODO  DE  ' + MaskEdit1.Text+ ' ATÉ ' +MaskEdit2.Text ;
    Qk_RelPro10.QuickRep1.Preview;
    Qk_RelPro10.Destroy;
    Qk_RelPro10 := NIL;
   end;
  Dm.ArqAux.Close;
...
procedure TF_RelPro10.MaskEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then Maskedit2.SetFocus;
end;

procedure TF_RelPro10.MaskEdit2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
    if key = 13 then
  begin
...
procedure TF_RelPro10.ComboBox1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TF_RelPro10.ComboBox2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then BitBtn1.SetFocus;
end;

procedure TF_RelPro10.ComboBox2Exit(Sender: TObject);
begin
//  TpBx := 'P';
  TpBx := Copy(ComboBox2.Text, 1,1);
end;

procedure TF_RelPro10.ComboBox1Enter(Sender: TObject);
begin
  SendMessage(ComboBox1.handle, CB_SHOWDROPDOWN, Integer(True), 0);
end;

procedure TF_RelPro10.ComboBox2Enter(Sender: TObject);
begin
  SendMessage(ComboBox2.handle, CB_SHOWDROPDOWN, Integer(True), 0);
end;

procedure TF_RelPro10.MaskEdit3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key = 13 then MaskEdit4.SetFocus;
end;

procedure TF_RelPro10.MaskEdit4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key = 13 then MaskEdit1.SetFocus;
end;

procedure TF_RelPro10.FormActivate(Sender: TObject);
begin
   F_RelPro10.MaskEdit3.SetFocus;
end;
