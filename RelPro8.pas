...
uses DMTD, Menu, QkRelPr8, FuncGeral, ProcGeral;
...
Procedure TF_RelPro8.GerarArquivoTxt(Sender : TOBject);
var
...
begin
 // if (StrToInt(Copy(ComboBox1.Text, 1,3)) = 6) then cam := F_Menu.Label27.Caption + 'Intercap\' else cam := F_Menu.Label27.Caption;
  ...
  arqu := cam + '\' + arqu;
  { Edson inicio }
  F_RelPro8.SaveDialog1.FileName := arqu;
  F_RelPro8.SaveDialog1.Execute;
  arqu := F_RelPro8.SaveDialog1.FileName;
  { Edson fim }
  AssignFile(F1, arqu);
  ReWrite(F1);
  linha := '';
...
procedure TF_RelPro8.BitBtn3Click(Sender: TObject);
begin
  Close;
end;

procedure TF_RelPro8.BitBtn2Click(Sender: TObject);
begin
  MaskEdit1.Clear;
...
procedure TF_RelPro8.BitBtn1Click(Sender: TObject);
var
  arq : String;
  contrato : String;
begin
  ...
  DM.Join.Close;
  DM.Join.Sql.Clear;
  DM.Join.Sql.Add('SELECT a.a_nome, a.contrato, a.parcelas, a.ven_parc, a.val_parc, a.arquivo, a.rl,');
  ...
  DM.Join.Sql.Add('b.nome, b.endereco, b.situacao, b.numero, b.complem, b.cep');
 // DM.Join.Sql.Add('FROM td');
  ...
  DM.Join.Sql.Add('and  (a.sit_prov1 = "")');
  ...
  if radiobutton1.checked = true then
   ...
   // DM.Join.Sql.Add('AND (a.data_prov1 is null))');
  ...
  if radiobutton2.checked = true then
   ...
   // DM.Join.Sql.Add('AND ((a.data_prov1 = null) or (a.data_prov1 = " ")))');
    If (CheckBox2.Checked = false) then
    DM.Join.Sql.Add('AND (a.ncliente   = ' + IntToStr(StrToInt(Copy(ComboBox1.Text, 1,3)))+ ')');
   end;
 ...
 // DM.Join.Sql.Add('AND (a.sit_prov3   = ' + Chr(39) + tpbx + Chr(39) + '))');}
  ...
  begin
    ...
    Dm.ArqAux.FieldByName('endereco').asString     := DM.Join.FieldByName('endereco').asString + ' '+ DM.Join.FieldByName('numero').asString + '  '+ DM.Join.FieldByName('complem').asString;
   // Dm.ArqAux.FieldByName('numero').asString       := DM.Join.FieldByName('numero').asString;
  //  Dm.ArqAux.FieldByName('complemento').asString  := DM.Join.FieldByName('complem').asString;
    ...
    if DM.Join.FieldByName('cod_ir1').asString <> '' then
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if DM.Join.FieldByName('cod_prov3').asString <> '' then
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              Dm.ArqAux.FieldByName('ocorrencia').asString := 'NEGATIVA';
              if DM.Join.FieldByName('cod_prov2').asString <> '' then
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                Dm.ArqAux.FieldByName('ocorrencia').asString := 'NEGATIVA';
                if DM.Join.FieldByName('cod_prov1').asString <> '' then
  ...
  begin
   // GerarArquivoTxt(F_RelPro8);
  end
  else
   begin
    Application.CreateForm(TQk_RelPro8, Qk_RelPro8);
    Qk_RelPro8.QrLabel1.Caption := F_Menu.Label29.Caption;
    Qk_RelPro8.QrLabel5.Caption := 'RELAÇÃO DAS NOTIFICAÇÕES EM ABERTO (INTERNET) -  NO PERÍODO  DE  ' + MaskEdit1.Text+ ' ATÉ ' +MaskEdit2.Text ;
    Qk_RelPro8.QuickRep1.Preview;
    Qk_RelPro8.Destroy;
    Qk_RelPro8 := NIL;
   end;
  Dm.ArqAux.Close;
...
procedure TF_RelPro8.MaskEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then Maskedit2.SetFocus;
end;

procedure TF_RelPro8.MaskEdit2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then
  begin
    ...
    if radiobutton1.checked = true then
     begin
      ...
      DM.ArqAux.Sql.Add('AND (data_prov1 is null)');
     end;
...
procedure TF_RelPro8.ComboBox1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TF_RelPro8.ComboBox2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then BitBtn1.SetFocus;
end;

procedure TF_RelPro8.ComboBox2Exit(Sender: TObject);
begin
//  TpBx := 'P';
  TpBx := Copy(ComboBox2.Text, 1,1);
end;

procedure TF_RelPro8.ComboBox1Enter(Sender: TObject);
begin
  SendMessage(ComboBox1.handle, CB_SHOWDROPDOWN, Integer(True), 0);
end;

procedure TF_RelPro8.ComboBox2Enter(Sender: TObject);
begin
  SendMessage(ComboBox2.handle, CB_SHOWDROPDOWN, Integer(True), 0);
end;

procedure TF_RelPro8.MaskEdit3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key = 13 then MaskEdit4.SetFocus;
end;

procedure TF_RelPro8.MaskEdit4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key = 13 then MaskEdit1.SetFocus;
end;

procedure TF_RelPro8.FormActivate(Sender: TObject);
begin
   F_RelPro8.MaskEdit1.SetFocus;
end;
