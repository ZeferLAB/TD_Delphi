...
procedure TF_RRecNF.BitBtn3Click(Sender: TObject);
begin
  close;
end;

procedure TF_RRecNF.BitBtn2Click(Sender: TObject);
begin
  maskedit1.text := '';
  maskedit2.text := '1';  
  maskedit3.text := '';
...
procedure TF_RRecNF.BitBtn1Click(Sender: TObject);
var
  ok : boolean;
  x, qq : Integer;
  prot : String;
begin
  ...
  begin
        ...
        begin
          ...
          Begin
            ...
            begin
              ...
              lin := lin + (pulo * 4);
              Printer.Canvas.TextOut(col, lin, '       São José dos Campos, _______/_______/________.');
              lin := lin + (pulo * 2);
              Printer.Canvas.TextOut(col, lin, '           ____________________________________');
              lin := lin + (pulo * 1);
              Printer.Canvas.TextOut(col, lin, '                Assinatura do Destinatário    ');
              printer.canvas.font.Name := old_font;
              Printer.Canvas.Font.Size := 14;
              Printer.Canvas.Font.Style := [fsBold];
              lin := lin + (pulo * 4);
              ...
              lin := lin + pulo + 30;
              Printer.Canvas.TextOut(col, lin, 'CERTIFICO E DOU FÉ, haver entregue na data acima, uma das vias da carta de');
              lin := lin + pulo + 30;
              Printer.Canvas.TextOut(col, lin, 'notificação registrada em microfilme sob número acima, juntamente com a(s)');
              lin := lin + pulo + 30;
              Printer.Canvas.TextOut(col, lin, 'fotocópia(s) anexa(s).');
              lin := lin + (pulo * 3);
    ...
    begin
      if (tabela.td.fieldbyname('data_p').asstring = F_RRecNF.Maskedit1.text) and (tabela.td.fieldbyname('protocolo').asstring = F_RRecNF.Maskedit3.text) then
      begin
        ok := true;
      end
      else ok := false;
    ...
    begin
      if tabela.td.fieldbyname('data_p').asstring = F_RRecNF.Maskedit1.text then
      begin
        ok := true;
      end
      else ok := false;
...
procedure TF_RRecNF.MaskEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TF_RRecNF.MaskEdit3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TF_RRecNF.MaskEdit2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 27 then maskedit1.setfocus;
