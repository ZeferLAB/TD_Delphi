...
var
  F_RETQAR1: TF_RETQAR1;
  quant : integer;

implementation

uses Menu, DMTD, QkEtqAR, QkEtqAR1, FuncGeral, ProcGeral;

{$R *.DFM}

procedure TF_RETQAR1.BitBtn3Click(Sender: TObject);
begin
  close;
end;

procedure TF_RETQAR1.MaskEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TF_RETQAR1.MaskEdit3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TF_RETQAR1.ComboBox1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 27 then maskedit1.setfocus;
...
procedure TF_RETQAR1.BitBtn1Click(Sender: TObject);
var
...
begin
  ...
  if (ValidNumero(ComboBox4.Text)) then
    begin
      quant := StrToInt(ComboBox4.Text);
    end
    else quant := 1;
...
//        Dm.ArqAux.Sql.Add(Chr(39) + 'RC' + Dm.Join.FieldByName('rl').asString + 'BR' + Chr(39) + ',');
 ...
 // Application.CreateForm(TQk_EtqAR1, Qk_EtqAR1);
  Qk_EtqAR1.QuickRep1.Preview;
 // Qk_EtqAR1.Destroy;
 // Qk_EtqAR1 := NIL;
  DestroeTabela(arq);
end;

procedure TF_RETQAR1.BitBtn2Click(Sender: TObject);
begin
  maskedit1.text    := '';
  maskedit3.text    := '';
  MaskEdit2.Text    := '';
...
procedure TF_RETQAR1.ComboBox2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 27 then maskedit1.setfocus;
...
procedure TF_RETQAR1.MaskEdit2Exit(Sender: TObject);
begin
  if (not ValidNumero(MaskEdit2.Text)) then MaskEdit2.Text := '0';
end;

procedure TF_RETQAR1.MaskEdit2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then ComboBox1.SetFocus;
end;

procedure TF_RETQAR1.MaskEdit3Exit(Sender: TObject);
begin
  if (not ValidNumero(MaskEdit3.Text)) then MaskEdit3.Text := '0';
end;

procedure TF_RETQAR1.FormActivate(Sender: TObject);
begin
  MaskEdit1.Clear;
  MaskEdit1.Text := DateToStr(Date);
end;

procedure TF_RETQAR1.ComboBox3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  //if key = 13 then BitBtn1.setfocus;
end;

procedure TF_RETQAR1.ComboBox4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then BitBtn1.setfocus;
end;
