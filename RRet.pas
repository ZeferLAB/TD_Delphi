...
uses DMTD, Menu, QkRetPad, QkRetI, FuncGeral, ProcGeral;
...
Procedure TF_RRetor.GerarArquivoTxt(Sender : TOBject);
var
...
begin
  ...
  arqu := cam + '\' + arqu;
  { Edson inicio }
  F_RRetor.SaveDialog1.FileName := arqu;
  F_RRetor.SaveDialog1.Execute;
  arqu := F_RRetor.SaveDialog1.FileName;
  { Edson fim }
  AssignFile(F1, arqu);
  ReWrite(F1);
  linha := '';
...
procedure TF_RRetor.BitBtn3Click(Sender: TObject);
begin
  Close;
end;

procedure TF_RRetor.BitBtn2Click(Sender: TObject);
begin
  MaskEdit1.Clear;
...
procedure TF_RRetor.BitBtn1Click(Sender: TObject);
var
  arq : String;
  contrato : String;
begin
  ...
  begin
    ...
    begin
      Application.CreateForm(TQk_RetPadrao, Qk_RetPadrao);
      Qk_RetPadrao.QrLabel1.Caption := F_Menu.Label29.Caption;
      Qk_RetPadrao.QrLabel5.Caption := 'RETORNO DAS NOTIFICAÇÕES CERTIFICADAS NO DIA ' + MaskEdit1.Text;
      Qk_RetPadrao.QuickRep1.Preview;
      Qk_RetPadrao.Destroy;
      Qk_RetPadrao := NIL;
    end;
  end;  
  Dm.ArqAux.Close;
...
procedure TF_RRetor.MaskEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then
  begin
...
procedure TF_RRetor.MaskEdit2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then ComboBox1.SetFocus;
end;

procedure TF_RRetor.ComboBox1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then ComboBox2.SetFocus;
end;

procedure TF_RRetor.ComboBox2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then BitBtn1.SetFocus;
end;

procedure TF_RRetor.ComboBox2Exit(Sender: TObject);
begin
  TpBx := 'P';
  TpBx := Copy(ComboBox2.Text, 1,1);  
end;

procedure TF_RRetor.ComboBox1Enter(Sender: TObject);
begin
  SendMessage(ComboBox1.handle, CB_SHOWDROPDOWN, Integer(True), 0);
end;

procedure TF_RRetor.ComboBox2Enter(Sender: TObject);
begin
