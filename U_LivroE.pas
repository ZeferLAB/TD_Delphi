...
uses DMTD, UQk_LivroE, menu, FuncGeral, ProcGeral;

{$R *.DFM}

procedure TF_LivroE.MaskEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  If Key = 13 then MaskEdit2.SetFocus;
end;

procedure TF_LivroE.ComboBox2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  If Key = 13 then BitBtn1.SetFocus;
end;

procedure TF_LivroE.BitBtn3Click(Sender: TObject);
begin
  Close;
end;

procedure TF_LivroE.BitBtn1Click(Sender: TObject);
var
  sign        : SisSign;
  certif      : Certificados;
  erros       : WideString;
...
begin
  ...
  dm.ArqAux.close;
  dm.ArqAux.sql.Clear;
  dm.ArqAux.sql.add('select a.data_p, a.registro, b.* from td a inner join td_livroe b ');
  dm.ArqAux.sql.add(' on(a.p_auxiliar = b.protocolo)');
  dm.ArqAux.sql.add(' where a.data_p between ' + QuotedStr(ConvData(MaskEdit1.Text)) + ' AND ' + QuotedStr(ConvData(MaskEdit2.Text)));
  dm.ArqAux.sql.add(' ORDER BY a.data_p, a.registro');
  dm.ArqAux.Open;

  if dm.ArqAux.RecordCount <> 0 then
  begin

    Application.CreateForm(TFQk_LivroE, FQk_LivroE);
    FQk_LivroE.Label4.Caption     := 'COM MOV';
    FQk_LivroE.Label5.Caption     := MaskEdit1.Text;
    FQk_LivroE.QrLabel18.Caption  := DataExtenso(MaskEdit1.Text, False, '') + ' a ' + DataExtenso(MaskEdit2.Text, False, '');
...
procedure TF_LivroE.MaskEdit2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then BitBtn1.SetFocus;
end;
