...
uses DMTD, Menu, QkRelPro, FuncGeral, ProcGeral;
...
Procedure TF_RelPro.GerarArquivoTxt(Sender : TOBject);
var
...
begin
 ...
 // arqu := Trim(Copy(ComboBox1.Text, 7, 3)) + TrimChar(Copy(maskedit1.text,1,5),'/') + Copy(combobox2.text,1,2) + '.TXT';
  arqu := cam + '\' + arqu;
  { Edson inicio }
  F_RelPro.SaveDialog1.FileName := arqu;
  F_RelPro.SaveDialog1.Execute;
...
procedure TF_RelPro.BitBtn3Click(Sender: TObject);
begin
  Close;
end;

procedure TF_RelPro.BitBtn2Click(Sender: TObject);
begin
  MaskEdit1.Clear;
...
procedure TF_RelPro.BitBtn1Click(Sender: TObject);
var
  arq : String;
  contrato : String;
begin
 ...
 // DM.Join.Sql.Add('AND (a.sit_prov3   = ' + Chr(39) + tpbx + Chr(39) + '))');}
  ...
  begin
    GerarArquivoTxt(F_RelPro);
  end
  else
   begin
    Application.CreateForm(TQk_RelPro, Qk_RelPro);
    Qk_RelPro.QrLabel1.Caption := F_Menu.Label29.Caption;
    Qk_RelPro.QrLabel5.Caption := 'RELATÓRIO GERAL DAS NOTIFICAÇÕES NO PERÍODO  DE  ' + MaskEdit1.Text+ ' ATÉ ' +MaskEdit2.Text ;
    Qk_RelPro.QuickRep1.Preview;
    Qk_RelPro.Destroy;
    Qk_RelPro := NIL;
   end;
  Dm.ArqAux.Close;
...
procedure TF_RelPro.MaskEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then Maskedit2.SetFocus;
end;

procedure TF_RelPro.MaskEdit2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
    if key = 13 then
  begin
...
procedure TF_RelPro.ComboBox1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TF_RelPro.ComboBox2KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = 13 then BitBtn1.SetFocus;
end;

procedure TF_RelPro.ComboBox2Exit(Sender: TObject);
begin
//  TpBx := 'P';
//  TpBx := Copy(ComboBox2.Text, 1,1);
end;

procedure TF_RelPro.ComboBox1Enter(Sender: TObject);
begin
  SendMessage(ComboBox1.handle, CB_SHOWDROPDOWN, Integer(True), 0);
end;

procedure TF_RelPro.ComboBox2Enter(Sender: TObject);
begin
//  SendMessage(ComboBox2.handle, CB_SHOWDROPDOWN, Integer(True), 0);
end;

procedure TF_RelPro.MaskEdit3KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
    if key = 13 then MaskEdit4.SetFocus;
end;

procedure TF_RelPro.MaskEdit4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
    if key = 13 then Maskedit1.setfocus;
end;

procedure TF_RelPro.FormActivate(Sender: TObject);
begin
    F_RelPro.MaskEdit3.SetFocus;
end;
