...
uses DMTD, Menu, FuncGeral, RRec;

{$R *.DFM}

procedure TQK_RecR.JustifyRichText(RichEdit: TRichEdit; AllText: Boolean);
const
...
procedure TQK_RecR.QRBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
...
begin
  ...
  dm.ArqAux1.sql.add('select * from daje where serie_daje_complementar ='+QuotedStr(Dm.ArqAux.FieldByName('serie_daje').asString));
  dm.ArqAux1.sql.add('and numero_daje_complementar ='+ QuotedStr(Dm.ArqAux.FieldByName('daje').asString));
  dm.ArqAux1.open;


  if  ((trim(Dm.ArqAux1.FieldByName('serie_daje').asString) <>'') and (trim(Dm.ArqAux1.FieldByName('numero_daje').asString) <>'' )) then
  begin
    Dm.Join.Close;
    Dm.Join.SQL.Clear;
    Dm.Join.SQL.Add('select * from daje where');
    Dm.Join.SQL.Add('serie_daje      = ' + QuotedStr(Dm.ArqAux1.FieldByName('serie_daje').asString));
    Dm.Join.SQL.Add('and numero_daje = ' + QuotedStr(Dm.ArqAux1.FieldByName('numero_daje').asString)+' and status <> '+QuotedStr('Isento')+'');
    Dm.Join.Open;
  end else
  begin
    Dm.Join.Close;
  ...
  dm.qryGenerico.SQL.Add('select serie_daje_complementar, numero_daje_complementar, emol_daje, tx_fisca_daje, fecom_daje,iddaje_tj,  def_pub_daje, vr_declarado, vr_daje, vr_pge, fmmpba from daje ');
  ...
  If iddaje_tj = 0 then iddaje_tj := Dm.Join.FieldByName('iddaje_tj').AsInteger;
  ...
  QrLabel7.Caption  := 'R$ ' + (StrCurr(Dm.ArqAux.FieldByName('valor_base').asCurrency, 20, True));
  ...
  if (dm.arqaux.fieldbyname('sequencia_averbacao').AsString <> '') and (dm.arqaux.fieldbyname('sequencia_averbacao').AsString <> '0') then
  begin
...
//QRLabel44.Caption := StringOfChar('_', Length(QrLabel26.Caption));
  ...
  Begin
    if (trim(dm.arqaux.fieldbyname('lva_reg').AsString) = 'X') then
    Begin
      If ((F_Menu.Label39.Caption = '05967436/000178') or (F_Menu.Label39.Caption = '05.967.436/0001-78')) then
      begin
        QRLabel32.Caption := 'LIVRO '+'A: '+trim(dm.arqaux.fieldbyname('livro_reg').AsString);
      end;
    ...
    Begin
      If ((F_Menu.Label39.Caption = '05967436/000178') or (F_Menu.Label39.Caption = '05.967.436/0001-78')) then
      begin
        QRLabel32.Caption := 'LIVRO '+'B: '+trim(dm.arqaux.fieldbyname('livro_reg').AsString);
      end;
    ...
    Begin
      If ((F_Menu.Label39.Caption = '05967436/000178') or (F_Menu.Label39.Caption = '05.967.436/0001-78'))  then
      begin
        QRLabel32.Caption := 'LIVRO '+'C: '+trim(dm.arqaux.fieldbyname('livro_reg').AsString);
      end;
  ...
  If ((F_Menu.Label39.Caption = '05967436/000178') or (F_Menu.Label39.Caption = '05.967.436/0001-78')
      or (F_Menu.Label39.Caption = '49765526000151') or (F_Menu.Label39.Caption = '49.765.526/0001-51')    ) then
  begin
     QRLabel30.Caption     := dm.arqaux.fieldbyname('folha_reg').AsString + ' - '+ F_RRec.total_pag;
  end;

  // livro F
  If dm.arqaux.fieldbyname('lvf').AsString = 'X' then
  Begin
    QRLabel_livro.Caption := 'Livro F';
    QRLabel4.Caption := trim(dm.arqaux.fieldbyname('livro_reg').AsString);
    QRLabel31.Caption := 'Folha:';
    QRLabel30.Caption := dm.arqaux.fieldbyname('folha_reg').AsString + '-'+F_RRec.total_pag;
  End;
...
procedure TQK_RecR.Quebra_linha_no_QRMemo1(v_Str: String);
var
