...
procedure TF_CorBaiE.FormActivate(Sender: TObject);
begin
  Dm.ArqAux.Close;
  Dm.ArqAux.Sql.Clear;
...
//  Dm.ArqAux.Sql.Add('p_auxiliar = ' + F_Pesq.Label16.Caption);
  ...
  MaskEdit8.Text      := Dm.ArqAux.FieldByName('data_not1').asString;
  MaskEdit2.Text      := Dm.ArqAux.FieldByName('hora_not1').asString;
  MaskEdit15.Text     := Dm.ArqAux.FieldByName('cod_ir1').asString;
  if Dm.ArqAux.FieldByName('cod_ir1').asString <> ''then
  ComboBox1.text      := Dm.ArqAux.FieldByName('cod_ir1').asString + ' - ' + Ver_Ocorrencia(Dm.ArqAux.FieldByName('cod_ir1').asString);
 ...
 { if Dm.ArqAux.FieldByname('sit_not1').asString = 'P' then
  ...
  MaskEdit1.Text      := Dm.ArqAux.FieldByName('data_not2').asString;
  MaskEdit3.Text      := Dm.ArqAux.FieldByName('hora_not2').asString;
  MaskEdit4.Text      := Dm.ArqAux.FieldByName('cod_ir2').asString;
  if Dm.ArqAux.FieldByName('cod_ir2').asString <> ''then
  ComboBox2.text      := Dm.ArqAux.FieldByName('cod_ir2').asString + ' - ' + Ver_Ocorrencia(Dm.ArqAux.FieldByName('cod_ir2').asString);
 ...
 { if Dm.ArqAux.FieldByname('sit_not2').asString = 'P' then
  ...
  MaskEdit5.Text      := Dm.ArqAux.FieldByName('data_not3').asString;
  MaskEdit6.Text      := Dm.ArqAux.FieldByName('hora_not3').asString;
  MaskEdit7.Text      := Dm.ArqAux.FieldByName('cod_ir3').asString;
  if Dm.ArqAux.FieldByName('cod_ir3').asString <> ''then
  ComboBox3.text      := Dm.ArqAux.FieldByName('cod_ir3').asString + ' - ' + Ver_Ocorrencia(Dm.ArqAux.FieldByName('cod_ir3').asString);
...
Procedure TF_CorBaiE.BuscaTextoCombo(Sender : TObject);
var
  x : Integer;
begin
  x := 0;
...
procedure TF_CorBaiE.BitBtn3Click(Sender: TObject);
begin
  Close;
end;

procedure TF_CorBaiE.Corrigir1Click(Sender: TObject);
var
...
begin
 ...
 // end;
  ...
  begin 
    campo1 := 'data_not' + Label19.Caption;
    campo2 := 'hora_not' + Label19.Caption;
    campo3 := 'cod_ir'   + Label19.Caption;
    campo4 := 'sit_not'  + Label19.Caption;
    Dm.ArqAux.Close;
    Dm.ArqAux.Sql.Clear;
    ...
    MaskEdit13.Text  := Dm.ArqAux.FieldByName(campo2).asString;
    MaskEdit14.Text  := Dm.ArqAux.FieldByName(campo3).asString;
    //MaskEdit9.Text  := Dm.ArqAux.FieldByName(campo4).asString;
    if Dm.ArqAux.FieldByName(campo3).asString <> ''then
    ComboBox4.Text  := Dm.ArqAux.FieldByName(campo3).asString + ' - ' + Ver_Ocorrencia(Dm.ArqAux.FieldByName(campo3).asString);
    Dm.ArqAux.Close;
...
procedure TF_CorBaiE.Excluir1Click(Sender: TObject);
var
 ...
 // end;
  ...
  begin 
    ...
    begin
      ...
      campo1 := 'data_not' + Label19.Caption;
      campo2 := 'hora_not' + Label19.Caption;
      campo3 := 'sit_not'  + Label19.Caption;
      campo4 := 'cod_ir'   + Label19.Caption;
      campo5 := 'obs_not'  + Label19.Caption;
      Dm.ArqAux2.Close;
   ...
   // Dm.ArqAux.Refresh;
    ...
    MaskEdit2.Text      := Dm.ArqAux.FieldByName('hora_not1').asString;
    MaskEdit15.Text     := Dm.ArqAux.FieldByName('cod_ir1').asString;
    if F_CorBaiE.MaskEdit15.text <> ''then
    ComboBox1.text      := Dm.ArqAux.FieldByName('cod_ir1').asString + ' - ' + Ver_Ocorrencia(Dm.ArqAux.FieldByName('cod_ir1').asString)
    else
    ComboBox1.text      := '';
    ...
    MaskEdit3.Text      := Dm.ArqAux.FieldByName('hora_not2').asString;
    MaskEdit4.Text      := Dm.ArqAux.FieldByName('cod_ir2').asString;
    if F_CorBaiE.MaskEdit4.text <> ''then
    ComboBox2.text      := Dm.ArqAux.FieldByName('cod_ir2').asString + ' - ' + Ver_Ocorrencia(Dm.ArqAux.FieldByName('cod_ir2').asString)
    else
    ComboBox2.text      := '';
    ...
    MaskEdit6.Text      := Dm.ArqAux.FieldByName('hora_not3').asString;
    MaskEdit7.Text      := Dm.ArqAux.FieldByName('cod_ir3').asString;
    if F_CorBaiE.MaskEdit7.text <> ''then
    ComboBox3.text      := Dm.ArqAux.FieldByName('cod_ir3').asString + ' - ' + Ver_Ocorrencia(Dm.ArqAux.FieldByName('cod_ir3').asString)
    else
    ComboBox3.text      := '';

    Atualiza_His(DateToStr(Date), TimeToStr(Time), 'T', F_Menu.Label1.Caption, 'Exclusão da ' + Label19.Caption + 'ª Baixa da Notificação em ' +  DateToStr(date),Trim(n_prot));
    BitBtn3.SetFocus;
  end;

end;

procedure TF_CorBaiE.BitBtn1Click(Sender: TObject);
var
  campo1, campo2, campo3, campo4, campo5 : String;
begin
 if (F_CorBaiE.MaskEdit12.text <> '  /  /    ') then
  begin
  if (MessageBox(Handle, PChar('Gravar correção da Baixa?'), 'BAIXA DA NOTIFICAÇÃO', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON1) = IdYes) then
    begin
      ...
      campo2 := 'hora_not' + Label19.Caption;
      campo3 := 'cod_ir'   + Label19.Caption;
      campo4 := 'obs_not'  + Label19.Caption;
      campo5 := 'sit_not'  + Label19.Caption;
      Dm.ArqAux.Close;
      ...
      begin
        ...
        Dm.ArqAux.FieldByName(campo2).asString := MaskEdit13.Text;
        Dm.ArqAux.FieldByName(campo3).asString := MaskEdit14.Text;
       ...
       // Atualiza_His(DateToStr(Date), TimeToStr(Time), 'T', F_Menu.Label1.Caption, 'Notificação baixada como ' + Ver_TipoBaixaNotificacao(Label5.Caption) + ' em ' + MaskEdit3.Text + ' pelo motivo ' + Copy(ComboBox1.Text, 7, 120), Label4.Caption);
        Atualiza_His(DateToStr(Date), TimeToStr(Time), 'T', F_Menu.Label1.Caption, 'Realizada correção na ' + Label19.Caption + 'ª Baixa da Notificação em ' +  DateToStr(date),Trim(n_prot));
        Dm.ArqAux.Next;
      end;
      Dm.ArqAux.Close;
      Dm.ArqAux.RequestLive := False;
...
procedure TF_CorBaiE.MaskEdit13KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key = 13 then MaskEdit14.SetFocus;
end;

procedure TF_CorBaiE.MaskEdit12KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key = 13 then MaskEdit13.SetFocus;
end;

procedure TF_CorBaiE.MaskEdit14Exit(Sender: TObject);
begin
 if (Not ValidNumero(MaskEdit14.Text)) OR
  ...
  begin
    ...
    BuscaTextoCombo(F_CorBaiE);
  end;
end;

procedure TF_CorBaiE.MaskEdit14KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   if key = 13 then ComboBox4.SetFocus;
end;

procedure TF_CorBaiE.ComboBox4Enter(Sender: TObject);
begin
  SendMessage(ComboBox4.handle, CB_SHOWDROPDOWN, Integer(True), 0);
end;

procedure TF_CorBaiE.ComboBox4Exit(Sender: TObject);
begin
   if ((Trim(MaskEdit14.Text) = '') OR (Trim(MaskEdit14.Text) = '0')) AND (Trim(ComboBox4.Text) <> '') then MaskEdit14.Text := Copy(ComboBox4.Text, 1,5);
end;

procedure TF_CorBaiE.ComboBox4KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
