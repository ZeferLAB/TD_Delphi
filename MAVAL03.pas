...
procedure Tf_maval03.QRSubDetail1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
//   if strtoint(copy(dm.arqaux.fieldbyname('cep').asstring,1,1)) < 2 then qrimage2.Enabled := true else qrimage2.Enabled := false;
     ...
     qrrichtext2.Lines.Add('O(s) valor(res) da(s)  prestação(ões) em atraso será(ão) acrescido(s) dos encargos contratuais e da mora');
     qrrichtext2.Lines.Add('até sua efetiva liquidação.');
     qrrichtext2.Lines.Add('');
     qrrichtext2.Lines.Add('Outrossim,  caso  V.Sa., já  tenha  pago  a(s) referida(s) prestação(ões),  objeto  da  presente  notificação,');
...
//   qrdbtext34.Enabled := true;
     ...
     qrrichtext1.Top := 343;
     qrrichtext1.Lines.Clear;
     qrrichtext1.Lines.Add('Vimos por meio desta, em decorrência do contrato supra firmado por V.Sa., notificá-lo para que  no prazo');
     qrrichtext1.Lines.Add('de 48 (quarenta e oito) horas, a contar do recebimento desta, efetue o  pagamento, no  endereço  abaixo');
     qrrichtext1.Lines.Add('indicado, da(s) prestação(ões) em atraso,  relativa(s) ao contrato referido.');
    ...
    { qrrichtext1.Lines.Add('Vimos por meio desta, em decorrência  do  contrato supra firmado por V.Sa., notificá-lo  para  que  no');
     ...
     qrrichtext2.Font.Size := 11;
     qrrichtext2.Lines.Clear;
     qrrichtext2.Lines.Add('O(s) valor(es) da(s) prestação(ões) em atraso será(ão) acrescido(s) dos encargos  contratuais e da  mora');
     qrrichtext2.Lines.Add('até sua efetiva liquidação.');
     qrrichtext2.Lines.Add('');
     qrrichtext2.Lines.Add('Outrossim,  caso  V.Sa.,  já tenha pago  a(s) referida(s)  prestação(ões), objeto  da  presente  notificação,');
     qrrichtext2.Lines.Add('rogamos considerar a mesma sem efeito, exibindo-nos entretanto, os devidos comprovantes.');
     qrrichtext2.Lines.Add('');
     qrrichtext2.Lines.Add('Alertamos que o  não pagamento da(s) prestação(ões) em  epígrafe, constituirá V.Sa., em  mora  para os');
     qrrichtext2.Lines.Add('efeitos legais (Decreto-Lei nº 911 de 01/10/69), possibilitando tomar  medidas judiciais  cabíveis, inclusive');
     qrrichtext2.Lines.Add('promover a imediata  ação de busca e apreensão do bem alienado fiduciariamente.');
     qrrichtext2.Lines.Add('');
     qrrichtext2.Lines.Add('Na expectativa de suas providências, subscrevemo-nos.');
   end;
...
//     qrdbtext34.Enabled := true;
     ...
     qrrichtext1.Top := 320;
     qrrichtext1.Lines.Clear;
     qrrichtext1.Lines.Add('Vimos através da presente, em decorrência do contrato supra firmado por V.Sa., notificá-lo(a)(s) para  que');
     qrrichtext1.Lines.Add('efetue o pagamento, no endereço abaixo indicado, da(s) parcela(s) e residual(ais) em atraso relativo(s) ao');
     qrrichtext1.Lines.Add('contrato em referência:');
     qrrichtext2.Font.Size := 11;
     qrrichtext2.Lines.Clear;
     qrrichtext2.Lines.Add('O(s) valor(es) da(s) parcela(s) e/ou residual(ais) acima será(ão) acrescido(s) dos encargos  contratuais  e');
     qrrichtext2.Lines.Add('da mora até sua efetiva liquidação.');
     qrrichtext2.Lines.Add('');
     ...
     qrrichtext2.Lines.Add(',das 9:00 às 18:00 horas, respondendo  por  eventual  débito  remanescente  de  contrato, sob  pena  de,');
     qrrichtext2.Lines.Add('assim não fazendo, vir a sofrer  as  medidas  judiciais  cabíveis, dentre  elas  a  Reintegração  de  Posse');
     qrrichtext2.Lines.Add('do Bem.');
     qrrichtext2.Lines.Add('');
     qrrichtext2.Lines.Add('Outrossim, alertamos que o não  atendimento a  esta notificação implicará na inscrição do nome de V.Sa.');
     qrrichtext2.Lines.Add('nos  Serviços   de    Cadastro   do   sistema    de    Proteção   ao   Crédito,  e  entidades   assemelhadas,');
...
//   qrdbtext34.Enabled := true;
     ...
     qrrichtext2.Lines.Add('horas, contadas  do   recebimento     desta,    efetue     o   pagamento     do    débito    ou    proceda     a  ');
     qrrichtext2.Lines.Add('imediata        devolução         do        veículo       objeto         do          arrendamento        mercantil,      na');
     qrrichtext2.Lines.Add(dm.arqaux.fieldbyname('end_banco').asstring );
     qrrichtext2.Lines.Add(',das 9:00 às 18:00 horas, respondendo  por  eventual  débito  remanescente  de  contrato, sob  pena  de,');
     qrrichtext2.Lines.Add('assim não fazendo, vir a sofrer  as  medidas  judiciais  cabíveis, dentre  elas  a  Reintegração  de   Posse');
     qrrichtext2.Lines.Add('do Bem.');
...
//   qrimage2.Picture.LoadFromFile('s:\td\assinatura\correio.jpg');
 //  if strtoint(copy(dm.arqaux.fieldbyname('cep').asstring,1,1)) < 2 then qrimage2.Enabled := true else qrimage2.Enabled := false;
     ...
     qrlabel9.caption  := 'Atenciosamente,';
     qrlabel63.caption := 'Prezado(a) Senhor(a),';
     qrimage1.Height := 121;
     qrimage1.Picture.LoadFromFile('s:\td\assinatura\assinatura fernando luz preta.jpg');
     qrlabel33.caption := 'Ref.: Contrato de Arrendamento Mercantil No.: ' + dm.arqaux.fieldbyname('contrato').asstring;
     ...
     qrrichtext2.Lines.Add('Informamos que V.Sa. está NOTIFICADO(A) a comparecer no(s) endereço(s), acima  relacionado(s),  para  efetuar o');
     qrrichtext2.Lines.Add('pagamento das obrigações vencidas com os acréscimos contratuais previstos,  DENTRO DO PRAZO MÁXIMO DE 2');
     qrrichtext2.Lines.Add('(DOIS) DIAS ÚTEIS,  a contar do recebimento da presente.');
     qrrichtext2.Lines.Add('Informamos ainda que, ultrapassado esse prazo sem a devida quitação, passarão a ser devidas todas as obrigações');
     qrrichtext2.Lines.Add('contratuais e demais  consequências de direito, inclusive o ajuizamento das medidas judiciais cabíveis.');
//   qrrichtext2.Lines.Add('judiciais cabíveis.');
     qrrichtext2.Lines.Add('Caso o pagamento tenha sido efetuado no período que  compreende a emissão da presente e o efetivo recebimento,');
     qrrichtext2.Lines.Add('pedimos desconsiderar esta notificação.');
   end;
...
//   qrimage2.Picture.LoadFromFile('s:\td\assinatura\correio.jpg');
 //  if strtoint(copy(dm.arqaux.fieldbyname('cep').asstring,1,1)) < 2 then qrimage2.Enabled := true else qrimage2.Enabled := false;
     ...
     qrrichtext2.Lines.Add('');
     qrrichtext2.Lines.Add('');
     qrrichtext2.Lines.Add('Nossos registros acusam ATRASO NO PAGAMENTO do Contrato de Crédito de responsabilidade de V.Sa, conforme');
     qrrichtext2.Lines.Add('acima indicado.');
     qrrichtext2.Lines.Add('Sentimos  informar-lhe, para  os  efeitos  previstos no parágrafo 2º, do art 2º, do Decreto-Lei Nº 911, de 01/10/69, que');
     qrrichtext2.Lines.Add('V.Sa. está NOTIFICADO(A) a comparecer no(s) endereço(s),  acima  relacionado(s), para  efetuar  o  pagamento das');
     qrrichtext2.Lines.Add('obrigações  vencidas  com os acréscimos contratuais previstos, DENTRO  DO  PRAZO  MÁXIMO DE  2 (DOIS) DIAS');
     qrrichtext2.Lines.Add('ÚTEIS, a contar do recebimento da presente.');
     qrrichtext2.Lines.Add('Informamos ainda que, ultrapassado esse prazo sem a devida quitação, passarão a  ser devidas todas as obrigações');
     qrrichtext2.Lines.Add('contratuais e demais consequências de direito, inclusive o ajuizamento das medidas judiciais cabíveis.');
//   qrrichtext2.Lines.Add('judiciais cabíveis.');
     qrrichtext2.Lines.Add('Caso o pagamento tenha sido efetuado no período que compreende a emissão da presente  e o efetivo  recebimento,');
